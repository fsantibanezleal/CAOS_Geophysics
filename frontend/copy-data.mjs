...
// 2) inline the pure analytic sources for optional browser-side inspection.
const pkg = join(ROOT, 'data-pipeline', 'geophysicslab');
if (existsSync(pkg)) {
  const sources = {};
  const walk = (dir, rel = '') => {
    for (const e of readdirSync(dir, { withFileTypes: true })) {
      ...
      else if (e.name.endsWith('.py')) sources[`geophysicslab/${r}`] = readFileSync(abs, 'utf-8');
    }
  };
  walk(pkg);
